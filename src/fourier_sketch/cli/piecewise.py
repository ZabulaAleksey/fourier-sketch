"""Localized local-image to explicit piecewise-curve diagnostic entry point."""
...
from fourier_sketch.application import LocalPathError, build_local_piecewise, validate_local_path
...
from fourier_sketch.render import render_piecewise_overlay_png
...
def main(argv: Sequence[str] | None = None) -> int:
    """Run PNG/JPEG -> Lee graph -> explicit PiecewiseCurve -> pen-up overlay."""
    ...
    try:
        ...
        result = build_local_piecewise(input_path, preprocessing)
        ...
        print(
            translator.text(
                "cli.piecewise_success",
                name=_safe_display_basename(output),
                status=conversion.status.value,
                segments=len(conversion.segments),
                boundaries=max(0, len(conversion.segments) - 1),
            )
        )
    except LocalPathError:
        _print_failure(translator, "piecewise.error.local_path")
        return 2
    except ImageInputError:
        _print_failure(translator, "piecewise.error.image_input")
        return 2
        ...
        _print_failure(translator, "piecewise.error.validation")
        return 2
    except FileExistsError:
        _print_failure(translator, "cli.piecewise_output_exists")
        return 2
    except OSError:
...
def _parser(translator: Translator) -> argparse.ArgumentParser:
    parser = _LocalizedArgumentParser(description=translator.text("cli.piecewise_description"))
    parser.add_argument("input", help=translator.text("cli.help.image_input"))
    parser.add_argument(
        "--output",
        default="piecewise.png",
        help=translator.text("cli.help.piecewise_output"),
    )
    parser.add_argument(
...
def _print_failure(translator: Translator, reason_key: str) -> None:
    print(
        translator.text("cli.piecewise_failed", reason=translator.text(reason_key)),
        file=sys.stderr,
    )
